...
def read_csv_as_nested_dict(filename, keyfield, separator, quote): #读取原始csv文件的数据，格式为嵌套字典
    ...
    """
    result1={}
#先定义一个字典然后将数据写入result
    with open(filename,newline="")as csvfile:
        csvreader=csv.DictReader(csvfile,delimiter=separator,quotechar=quote)
        for row in csvreader:#遍历csvreader并写入
            rowid=row[keyfield]
            result1[rowid]=row
...
#gdp_countries,嵌套字典，该字典的键为国家代码，键的值为具体国名
    set1=set(gdp_countries)#将字典和集合写入元组
    tuple1=(dict1,set1)
    return tuple1
...
def build_map_dict_by_name(gdpinfo, plot_countries, year):
    ...
    for plot_countries_code in plot_countries :#遍历plot_countries
        for isp_country_code in gdpinfo :
            
            if gdpinfo[isp_country_code]["Country Name"] == plot_countries[plot_countries_code] :
                
                gdp_number = ''
                country_imformation2=[]
                
                for country_imformation in gdpinfo[isp_country_code] :
                    country_imformation2.append(country_imformation)
                
                for year_1 in country_imformation2[4:-1] :
                ...
                if gdp_number == '' :
                    ...
                    if gdpinfo[isp_country_code][year] != '' :
                        ...
                        gdp_num=math.log10(float((gdpinfo[isp_country_code][year])))#应用math.log10函数
    ...
    tuple_2=(countries_year_gdp,set2,set3)
...
def render_world_map(gdpinfo, plot_countries, year, map_file): #将具体某年世界各国的GDP数据(包括缺少GDP数据以及只是在该年缺少GDP数据的国家)以地图形式可视化
    ...
    countries_1,countries_2,countries_3 = build_map_dict_by_name(gdpinfo, plot_countries, year)
#写入有数据国家及GDP数据，和绘图库中没数据的国家，以及在世界银行无GDP数据国家
    worldmap_chart = pygal.maps.world.World()
    worldmap_chart = pygal.maps.world.World()
    worldmap_chart.title = '世界银行%s年国家GDP数据'%(year)
    worldmap_chart.add('该年在绘图库及世界银行有数据的国家及其GDP数据', countries_1)
    worldmap_chart.add('在绘图库中没有数据的国家',countries_2)
    worldmap_chart.add('该年在世界银行没有GDP数据的国家',countries_3)
...
def test_render_world_map(year):  #测试函数
    ...
    gdp_csv=read_csv_as_nested_dict(gdpinfo['gdpfile'], gdpinfo['country_code'], gdpinfo['separator'], gdpinfo['quote'])#写入

    pygal_countries = pygal.maps.world.COUNTRIES   # 获得绘图库pygal国家代码字典
    
    isp_countries=[]
    for isp_country_code in gdp_csv :
        isp_countries.append(gdp_csv[isp_country_code]["Country Name"])#读取pygal.maps.world中国家代码信息（为字典格式），其中键为pygal中各国代码，值为对应的具体国名
...
while float(year) < 1960 or float(year) > 2015 :#识别国家是否能向可视化结果中写入GDP数据
